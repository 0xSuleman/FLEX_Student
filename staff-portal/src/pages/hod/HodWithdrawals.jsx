...
import { FileX, ChevronUp, ChevronDown, Check, X, RefreshCw, AlertTriangle, CheckCircle2 } from 'lucide-react'
import api from '../../services/api'
import { PageHeader } from '../../components/PageShell'

const TONE = {
  PENDING_HOD: 'bg-mustard text-ink',
  APPROVED:    'bg-moss text-cream',
  REJECTED:    'bg-bad text-bone',
}

export default function HodWithdrawals() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await api.get('/hod/withdrawals')
  ...
  const decide = async (id, action) => {
    ...
    setActing(true)
    try {
      await api.post(`/hod/withdrawals/${id}/decide`, { action, remarks: r })
    ...
    <div className="space-y-5 max-w-[1500px]">
      ...
      {loading ? (
        ...
        <div className="chunky-card p-12 text-center">
          ...
          <p className="text-cocoa font-bold">No withdrawals awaiting HOD decision.</p>
        </div>
      ) : (
                ...
                {open && (
                  <div className="p-5 space-y-3">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      <Field label="Faculty Recommendation" value={r.facultyRecommendation || '—'} />
                      <Field label="Course" value={`${r.courseCode} · ${r.section}`} />
                      <Field label="Semester" value={r.semester} />
                    </div>
                    {r.facultyRemarks && (
                      <div>
                        <div className="text-[10px] font-extrabold text-coffee uppercase tracking-widest mb-1.5">&gt; Faculty Remarks</div>
                        <div className="bg-bone border-2 border-ink rounded-md p-3 text-sm text-ink">{r.facultyRemarks}</div>
                      </div>
                    )}
                    <div>
