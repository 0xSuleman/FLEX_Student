...
export default function CourseRegistration() {
  ...
  const now = new Date()
  const regEnd = new Date('2026-02-01')
  const registrationClosed = now > regEnd
      ...
      {registrationClosed ? (
        <div className="chunky-card p-5 bg-mustard/20 cascade-in" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-display text-[10px] text-ink uppercase mb-2">Registration Closed</h3>
              <p className="text-xs text-cocoa font-medium">
                The registration period for {SEMESTER} has ended (Jan 14 – Feb 1, 2026).
                Contact the registrar's office for late registration inquiries.
              </p>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="chunky-card overflow-hidden cascade-in" style={{ animationDelay: '0.15s' }}>
              <div className="px-4 sm:px-5 py-3 sm:py-3.5 border-b-2 border-ink bg-tan">
                <h3 className="heading-retro text-xs sm:text-sm truncate min-w-0">Available Courses</h3>
              </div>
